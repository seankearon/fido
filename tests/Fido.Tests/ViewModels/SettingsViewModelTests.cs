...
{
    [Test]
    public async Task ApplyTo_leaves_the_new_branch_repo_list_untouched()
    {
        // Settings no longer edits NewBranchRepos (the placement flow went with the redesign), but a
        // configured list must survive a load/apply round-trip in case the feature returns.
        var vm = new SettingsViewModel();
        var cfg = new AppConfig { NewBranchRepos = new() { @"C:\src\a", @"C:\src\b" } };
        vm.LoadFrom(cfg);
        vm.ApplyTo(cfg);

        await Assert.That(cfg.NewBranchRepos.Count).IsEqualTo(2);
    }
    ...
    [Test]
    public async Task ShowTargetInWindowTitle_round_trips_through_load_and_apply()
    {
        var vm = new SettingsViewModel();
        vm.LoadFrom(new AppConfig());   // on by default

        await Assert.That(vm.ShowTargetInWindowTitle).IsTrue();

        vm.ShowTargetInWindowTitle = false;   // unticking the box

        var cfg = new AppConfig();
        vm.ApplyTo(cfg);
        await Assert.That(cfg.ShowTargetInWindowTitle).IsFalse();

        vm.LoadFrom(cfg);                     // and a config with it off loads unticked
        await Assert.That(vm.ShowTargetInWindowTitle).IsFalse();
    }
