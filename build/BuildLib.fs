...
// Adapted from Klippy.Build's BuildLib, which in turn came from Pirform.Build. Only the
// general-purpose parts are here: running commands, moving files about, asking git
// questions, and the stage/timing scaffolding the build reports with. Everything that
// knows anything about Fido lives in Program.fs.
//
// Declarations are ordered so nothing refers forward. `module rec` is kept only so this
// file stays interchangeable with Klippy's - if these are ever pulled into a shared repo,
// this one can be split into files without any reordering first.
...
/// A build log needs "1m 04s", not prose, so Humanizer is not worth carrying for one call.
...
/// Fido has no AssemblyInfo.cs - every project is SDK-style and generates its own assembly
/// attributes, so adding one would be a CS0579 duplicate-attribute error. A root
/// Directory.Build.props is the equivalent, and it is what Parcel reads for the bundle and
/// installer metadata.
///
/// Directory.Build.props is imported before the project body, so anything src/Fido.csproj
/// sets for itself (Product, Description, AssemblyTitle) still wins; the values that matter
/// here are the ones the csproj leaves alone - Version above all.
let createDirectoryBuildPropsFile (fullPath: string) (details: BuildDetails) =
    $"""<Project>
    <!-- Generated by Fido.Build; edits here are overwritten and reverted by the build. -->
    <PropertyGroup>
